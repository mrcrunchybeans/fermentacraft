...
class SyncCoordinator {
  ...
  Future<Result<void, Exception>> initialize({
    ...
    try {
      ...
      if (batchResult.isFailure) {
        return Failure(Exception('Failed to initialize batch repository: ${batchResult.error}'));
      }
      
      final recipeResult = await _recipeRepository!.initialize();
      if (recipeResult.isFailure) {
        return Failure(Exception('Failed to initialize recipe repository: ${recipeResult.error}'));
      }
  ...
  Future<Result<SyncSummary, Exception>> performFullSync() async {
    ...
    try {
      ...
      log('Starting full sync', name: 'SyncCoordinator');
      
      final summary = SyncSummary();
      
      // Sync batches
      final batchResult = await _batchRepository!.sync();
      ...
      // Sync recipes
      final recipeResult = await _recipeRepository!.sync();
      summary.recipeSyncSuccess = recipeResult.isSuccess;
      if (recipeResult.isFailure) {
        summary.errors.add('Recipe sync failed: ${recipeResult.error}');
      }
      
      // Update sync time if any succeeded
      if (summary.batchSyncSuccess || summary.recipeSyncSuccess) {
        _lastFullSyncTime = DateTime.now();
        AppLogger.instance.info('Full sync completed', 
            details: {'errors': summary.errors.length});
      }
      ...
      log('Full sync failed: $e', error: e, name: 'SyncCoordinator');
      _emitSyncEvent(SyncEvent.failed(e.toString()));
      return Failure(Exception('Full sync failed: $e'));
    } finally {
      _isSyncing = false;
  ...
  Future<Result<void, Exception>> clearCacheAndResync() async {
    ...
    try {
      ...
      log('Clear cache and resync failed: $e', error: e, name: 'SyncCoordinator');
